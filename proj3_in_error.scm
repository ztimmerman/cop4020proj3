...
(define smallestGrade
  (lambda (roster min)
    (cond
      ((equal? roster '()) min)
      ((< (string->number(car (cdr (cdr (car roster))))) (string->number (car (cdr (cdr min)))))
        (smallestGrade (cdr roster) (car roster)))
      (else
        (smallestGrade (cdr roster) min))
    )
  )
)
...
(define smallestName
  (lambda (roster min)
    (cond
      ((equal? roster '()) min)
      ((string< (car (cdr (car roster))) (car (cdr min)))
        (smallestName (cdr roster) (car roster))
      )
      (else
        (smallestName (cdr roster) min)
      )
    )
  )
)
...
(define selectSortName
  (lambda (roster)
    (begin
      (cond ((null? roster) '())
            (else (cons (smallestName roster (car roster))
                (selectSortName (sortRemove roster (smallestName roster (car roster)))))
            )
      )
    )
  )
...
(define selectSortGrade
  (lambda (roster)
    (begin
      (cond ((null? roster) '())
            (else (cons (smallestGrade roster (car roster))
                (selectSortGrade (sortRemove roster (smallestGrade roster (car roster)))))
            )
      )
    )
  )
...
(define performtask
  (lambda (n roster)
    (cond ((= n 0) (begin
          ...
          ((equals? n "G") (begin
                    (display "\nDisplaying by Grade.\n")
                    (display (selectSortGrade roster))
                    (newline)
                    (menu roster)
                    ))
          ((equals? n "N") (begin
                    (display "\n"Displaying by Name.\n")
                    (display (selectSortName roster))
                    (newline)
                    (menu roster)
                    ))
          (else    (begin
...
(define menu
  (lambda (roster)
    (begin
      ...
      (display "\t7. Exit\n")
      (display "\tG. Display roster sorted by Grade.\n")
      (display "\tN. Display roster sorted by Name.")
      (display "\n>> ")
